...
library(plotly)
library(lubridate)
inflation <- read_csv('~/blogdown/econ03ge3a/working/2.GrowthAndDistributionRegimes/india-inflation-rate-cpi.csv',
         skip = 16) 
(inflation1 <- inflation %>%   
  select(1:2)  %>% 
    mutate(Year = as.numeric(year(date))) %>% 
      mutate(event = if_else(Year == 1962, 'Chinese War',
                  if_else(Year == 1965, 'Pakistan War',
...
Ref: [Inflation In India: Status and Issues by Y.V. Reddy](https://rbidocs.rbi.org.in/rdocs/Bulletin/PDFs/9319.pdf) 
